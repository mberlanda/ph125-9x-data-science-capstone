...
```{r test_train_sets, messages=FALSE}
...
p_rating_2003_distribution <- train_set %>% group_by(rating, reviewYear) %>% count() %>% 
...
ratingByMovieYear <- train_set %>% group_by(movieYear) %>% count() %>% mutate(perc = round(n/nrow(train_set), digits = 3))
...
ratingAvgByMovieYear <- train_set %>% group_by(movieYear) %>% summarize(avg=mean(rating)) 
...
> It is important to note that all users and movies both in `test_set` and in `validation` are included
> by design in the `train_set`. This allows us to stretch the impact of the section b. of the winning
...
rmse_results <- tibble(method = "Model 0: Average", RMSE = naive_rmse, lambda = 0)
```
...
model_1_lambdas_rmse <- sapply(lambdas, function(l) {
  RMSE(predict_model_1(test_set, l), test_set$rating)
})
l <- lambdas[which.min(model_1_lambdas_rmse)]

l_min <- max(0, l-l_step)
l_max <- max(0, l+l_step)
l_step <- l_step/10
lambdas <- seq(l_min, l_max, l_step)
...
if(l != 0) {
  rmse_results <- bind_rows(rmse_results,
                          data_frame(
                            ...
                            RMSE = which.min(model_1_lambdas_rmse),
                            lambda = l
                          ))
}

rm(lambdas, l, l_min, l_max)
```
...
model_2_rmse <- RMSE(predict_model_2(test_set, 0), test_set$rating)
rmse_results <- bind_rows(rmse_results,
...
model_2_lambdas_rmse <- sapply(lambdas, function(l) {
  RMSE(predict_model_2(test_set, l), test_set$rating)
})
...
if(l != 0) {
  rmse_results <- bind_rows(rmse_results,
                          data_frame(
                            ...
                            RMSE = which.min(model_2_lambdas_rmse),
                            lambda = l
                          ))
}
rm(lambdas, l_min, l_max)
``` 
...
```{r test_set_rmse}
#
RMSE(predict_model_2(validation, l), validation$rating) 
